    ...
    switch (schemakey) {
        ...
        switch (this.schema[key][schemakey]) {
          case "string":
            if (/^[a-zA-Z ]+$/.test(value) === false) {
              this.isValid = false
              this.errors[key] = `Only alphabets are allowed.`
            }
            break
          case "number":
            if (/^[0-9]+$/.test(value) === false) {
              this.isValid = false
              this.errors[key] = `Only digits are allowed.`
            }
            break
          case "email":
      ...
      case "lt":
        if (this.schema[key].lt <= parseInt(value)) {
          this.isValid = false
          this.errors[key] = `Value should be less than ${this.schema[key].lt}.`
        }
        break
      case "lte":
        if (this.schema[key].lte + 1 < parseInt(value)) {
          this.isValid = false
          this.errors[key] = `Value should be less than or equal to ${this.schema[key].lte}.`
        }
        break
      case "gt":
        if (this.schema[key].gt >= parseInt(value)) {
          this.isValid = false
          this.errors[key] = `Value should be greater than ${this.schema[key].gt}.`
        }
        break
      case "gte":
        if (this.schema[key].gte + 1 >= parseInt(value)) {
          this.isValid = false
          this.errors[key] = `Value should be greater than or equal to ${this.schema[key].gte}.`
        }
        break
      case "customRegExp":
        if (this.schema[key].customRegExp.test(value) === false) {
          this.isValid = false
          this.errors[key] = `This filed is invalid.`
        }
    }
  }
  validate() {
